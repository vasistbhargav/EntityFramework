...
namespace Microsoft.EntityFrameworkCore.Migrations
{
    public class SqlServerMigrationsSqlGenerator : MigrationsSqlGenerator
    {
            ...
            MigrationCommandListBuilder builder)
        {
            ...
            builder
                .AppendLine("DECLARE @db_name NVARCHAR(MAX)")
                .AppendLine("SET @db_name = DB_NAME()")
                .AppendLine("IF SERVERPROPERTY('EngineEdition') <> 5 AND SERVERPROPERTY('IsXTPSupported') = 1");
            using (builder.Indent())
            {
                builder
                    .AppendLine("BEGIN")
                ...
                {
                    builder
                        .Append("SELECT * FROM [sys].[filegroups] [FG] ")
                        .Append("JOIN [sys].[database_files] [F] ON [FG].[data_space_id] = [F].[data_space_id] ")
                        .AppendLine("WHERE [FG].[type] = 'FX' AND [F].[type] = 2)");
                }
                ...
                using (builder.Indent())
                {
                    builder
                        ...
                        .AppendLine()
                        .AppendLine("IF (@path IS NOT NULL)");
                    using (builder.Indent())
                    {
                        builder
                            .AppendLine("BEGIN")
                            .AppendLine("DECLARE @filename NVARCHAR(MAX), @folder NVARCHAR(MAX), @new_path NVARCHAR(MAX), @fg_name NVARCHAR(MAX)")
                            .AppendLine("SET @filename = right(@path, charindex('\\', reverse(@path)) - 1)")
                            .AppendLine("SET @folder = left(@path, len(@path) - len(@filename))")
                            .AppendLine("SET @filename = left(@filename, len(@filename) - charindex('.', reverse(@filename)))")
                            .AppendLine("SELECT TOP(1) @fg_name = name FROM [sys].[filegroups] WHERE type = 'FX'")
                            .AppendLine()
                        ...
                        using (builder.Indent())
                        {
                            builder
                                .AppendLine("BEGIN")
                                .AppendLine("SET @fg_name = @filename + '_FG'")
                                .AppendLine("EXEC(N'ALTER DATABASE [' + @db_name + '] ADD FILEGROUP [' + @fg_name + '] CONTAINS MEMORY_OPTIMIZED_DATA;')")
                                .AppendLine("END");
                        }

                        builder
                            .AppendLine()
                            .AppendLine("SET @filename = @filename + '_MOD'")
                            .AppendLine("SET @new_path = @folder + @filename")
                            .AppendLine()
                            .AppendLine("EXEC(N'");
                        ...
                        {
                            builder
                                .AppendLine("ALTER DATABASE [' + @db_name + ']")
